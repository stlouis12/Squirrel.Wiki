...
public class FileService : BaseService, IFileService
{
    ...
    public async Task<Result<FileDetailsDto>> GetFileDetailsAsync(Guid fileId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var dto = Mapper.Map<FileDetailsDto>(file);
            
            // Set FolderPath (requires async operation, so can't be done in AutoMapper)
            dto.FolderPath = await BuildFolderPathAsync(file.FolderId, cancellationToken);
